	...
	if (gore)
	{
		...
		if ( gore->tex[TS.lod] )
		{
			if (gore->tex[TS.lod]->verts)
				R_Free(gore->tex[TS.lod]->verts);
			if (gore->tex[TS.lod]->indexes)
				R_Free(gore->tex[TS.lod]->indexes);
...
/*
#ifdef _G2_GORE
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd, CCollisionRecord *collRecMap, int entNum, int eG2TraceType, int useLod, float fRadius, float ssize,float tsize,float theta,int shader, SSkinGoreData *gore, qboolean skipIfLODNotMatch)
#else
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd, CCollisionRecord *collRecMap, int entNum, int eG2TraceType, int useLod, float fRadius)
#endif
*/
#ifdef _G2_GORE
...
qboolean G2_SaveGhoul2Models(CGhoul2Info_v& ghoul2, char **buffer, int *size)
{
	...
	*size = 0;
	...
	int ghoul2BlockSize = (intptr_t)&ghoul2[0].mTransformedVertsArray - (size_t)&ghoul2[0].mModelindex;
